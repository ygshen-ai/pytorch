...
std::optional<std::pair<std::string, uint64_t>> libraryFor(void* addr) {
    ...
    DWORD get_filename_ok = GetModuleFileNameW(
        h_module,
        module_full_path,
        max_name_len
    );

    if (get_filename_ok == 0) {
        return std::nullopt;
    }
    ...
    return std::make_optional(std::make_pair(module_base_name, offset));
}
...
std::vector<Frame> symbolize(const std::vector<void*>& frames, Mode mode) {
    ...
    for (void* frame_addr : frames) {
        ...
        if (SymFromAddr(hProcess, addr, &disp, pSymbol)) {
            ...
            if (disp > 0) {
                detail.funcname += "+0x" + std::to_string(disp);
            }
        }
